...
class mainViewController: BaseVC{
    ...
    private lazy var summ: UILabel = {
        let label = UILabel()
        label.textAlignment = .left
        label.font = UIFont(name: "Neucha", size: 20)
        label.text = "Введите сумму на чеке:"
        label.textColor = UIColor.black
        
        return label
    }()
    
    private lazy var summOfPayment: UITextField = {
        let people = UITextField()
        people.backgroundColor = .white
        people.placeholder = "Сколько нужно платить?"
        people.font = UIFont(name: "Neucha", size: 14)
        people.textColor = UIColor(red: 0, green: 0, blue: 0, alpha: 1)
        people.borderStyle = .roundedRect
        //        people.delegate = self
    ...
    private lazy var resultHowMuchPay: UITextField = {
        let people = UITextField()
        people.backgroundColor = .white
    ...
    override func setupViews() {
        ...
        view.addSubview(summOfPayment)
        view.addSubview(result)
        view.addSubview(resultHowMuchPay)
    ...
    override func setupConstrains() {
        ...
        summ.snp.makeConstraints(){
            $0.top.equalTo(peopleTF.snp.bottom).offset(computedHeight(20))
            $0.leading.equalToSuperview().offset(computedWidth(20))
        }
        
        summOfPayment.snp.makeConstraints{
            $0.top.equalTo(summ.snp.bottom).offset(computedHeight(4))
            $0.leading.trailing.equalToSuperview().inset(computedWidth(16))
        }
        
        result.snp.makeConstraints(){
            $0.top.equalTo(summOfPayment.snp.bottom).offset(computedHeight(20))
            $0.leading.equalToSuperview().offset(computedWidth(20))
        }
        
        resultHowMuchPay.snp.makeConstraints{
            $0.top.equalTo(result.snp.bottom).offset(computedHeight(4))
            $0.leading.trailing.equalToSuperview().inset(computedWidth(16))
        }
    }
}
